...
let category2 [n] (t: f64) (v: [n]f64): (f64,f64,i64,[]scenario) =
  ...
  let scenario (n: f64) (m: moments): scenario =
    (favourable n m, moderate n m, unfavourable n m, stress n m s z_alpha)

  in if (t <= 1) then (var, vev, mrm, [scenario (days*t) m])
    else if (t <= 3)
        then (var, vev, mrm, [scenario 1.0 m, scenario (days*t)   m])
        else (var, vev, mrm, [scenario 1.0 m, scenario (days*t/2) m, scenario (days*t) m])
